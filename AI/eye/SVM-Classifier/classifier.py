...
# Check if the svm_model.pkl file already exists
model_file_path = os.path.join(current_dir, 'eye_model.pkl')
if not os.path.exists(model_file_path):
    # Load training data and labels
    training_data = []
    training_labels = []
    categories = ['cherry', 'entropion', 'normal']
    ...
    eye_model = SVC(kernel='linear', C=10.0)  # Adjust C value for tuning
    ...
    val_predictions = eye_model.predict(X_val)
    ...
    with open(model_file_path, 'wb') as model_file:
        pickle.dump(eye_model, model_file)
    print("Eye model trained and saved.")
else:
    print("eye_model.pkl already exists. Skipping training.")
...
for label, category in enumerate(categories):
    ...
    for image_file in image_files:
        ...
        predicted_label = eye_model.predict([hog_features])[0]
