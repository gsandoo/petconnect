...
model_file_path = os.path.join(current_dir, 'disease_model.pkl')
if not os.path.exists(model_file_path):
    ...
    disease_model = SVC(kernel='linear', C=10.0)  # Adjust C value for tuning
    ...
    val_predictions = disease_model.predict(X_val)
    ...
    with open(model_file_path, 'wb') as model_file:
        pickle.dump(disease_model, model_file)
...
for label, category in enumerate(categories):
    ...
    for image_file in image_files:
        ...
        predicted_label = disease_model.predict([hog_features])[0]
